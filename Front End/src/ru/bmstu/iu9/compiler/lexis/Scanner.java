...
/**
 * Это основной класс, используемый {@link ru.bmstu.iu9.compiler.lexis.Lexer Lexer}.
...
class Scanner implements Iterable<Token> {
     ...
     */
    Token nextToken() throws LexisException {
        ...
        switch (iterator.current().value()) {
            ...
            case '\'':
                ...
                if (iterator.current().value() == '\'') {
                    ...
                    throw (LexisException)new InvalidCodePointException(iterator.current())
                            .initPosition(iterator.current().position())
                            .Log("ru.bmstu.iu9.compiler.lexer");
                }
            case '~':
                iterator.advance(1);
                ...
                if(Character.isDigit(iterator.current().value())) {
                    ...
                    if (iterator.current().value() == '0') {
                        ...
                        if (iterator.current().value() == 'x' || 
                            ...
                            checkIfNextIsNotLetter(current);
     ...
     *         противном случае
     */
    boolean skipWhitespacesAndComments() {
        while (iterator.hasNext() && 
                Character.isWhitespace(iterator.current().value())) {
            iterator.next();
        }
    ...
    void checkIfNextIsNotLetter(CodePoint current) throws LexisException {
        if (Character.isLetter(iterator.current().value())) {
            while(iterator().hasNext() &&
                    Character.isLetterOrDigit(iterator.next().value())) {
                continue;
            }

            throw (LexisException)new InvalidLexemeException(
                        text.substring(
                            current.position().index(),
                            iterator.current().position().index()
                        )
